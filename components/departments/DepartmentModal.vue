...
// Biến lưu tạm giá trị khi người dùng đang gõ tên chức danh mới
...
watch(() => props.departmentData, (newData) => {
  if (newData && newData.id) {
    formData.value = {
      ...newData,
      // Clone mảng position để tránh lỗi tham chiếu chéo, nếu không có thì gán mảng rỗng
      position: Array.isArray(newData.position) ? [...newData.position] : []
    }
  } else {
    formData.value = {
      name: '',
  ...
  newPositionInput.value = '' // Reset ô nhập chức danh
}, { immediate: true })
...
const selectManager = (emp) => {
  ...
  formData.value.employeeID = emp.employeeCode // Giả sử emp có trường id hoặc employeeCode
  isDropdownOpen.value = false
  searchManagerQuery.value = ''
}

// ==========================================
// LOGIC QUẢN LÝ MẢNG CHỨC DANH (POSITION)
// ==========================================
const addPosition = () => {
  const posName = newPositionInput.value.trim()
  // Chỉ thêm nếu có chữ và chưa tồn tại trong mảng
  if (posName && !formData.value.position.includes(posName)) {
    formData.value.position.push(posName)
  }
  newPositionInput.value = '' // Xóa trống ô nhập sau khi thêm
}

const removePosition = (index) => {
  formData.value.position.splice(index, 1)
}
// ==========================================
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg mx-4 p-6 z-50">
      ...
      <div class="space-y-4 overflow-y-auto pr-2 flex-1 custom-scrollbar">
        ...
        <div v-if="isEditMode" class="relative">
          ...
          <div @click="isDropdownOpen = !isDropdownOpen"
            class="w-full px-4 py-2 border rounded-lg cursor-pointer bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white flex justify-between items-center hover:border-blue-500 transition-colors">
            <span :class="{ 'text-gray-400': !formData.employeeName }">{{ formData.employeeName || '-- Vui lòng chọn trưởng phòng --' }}</span>
          ...
          <div v-if="isDropdownOpen"
            class="absolute z-20 w-full mt-1 bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-lg shadow-xl overflow-hidden">
            <div class="p-2 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
              <input v-model="searchManagerQuery" type="text" placeholder="Tìm kiếm..."
                class="w-full px-3 py-1.5 text-sm border rounded bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                autofocus />
            </div>
            <ul class="max-h-48 overflow-y-auto">
              <li v-for="emp in filteredEmployees" :key="emp.id" @click="selectManager(emp)"
                class="px-4 py-2 text-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors flex flex-col">
              ...
              <li v-if="filteredEmployees.length === 0" class="px-4 py-3 text-sm text-gray-500 text-center">Không tìm
                thấy nhân viên.</li>
            </ul>
          </div>
        </div>
        ...
        <div v-else class="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg text-sm text-blue-700 dark:text-blue-300">
          <span class="font-bold">💡 Mẹo:</span> Sau khi tạo phòng ban, hãy sang trang Nhân sự để thêm nhân viên vào
          phòng này. Sau đó bạn mới có thể bổ nhiệm Trưởng phòng.
        </div>
        ...
        <div class="border-t dark:border-gray-700 pt-4 mt-2">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-if="formData.position.length === 0" class="text-sm text-gray-400 italic"> Chưa có chức danh nào được thêm. </span>
            <span v-for="(pos, index) in formData.position" :key="index"
              class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 rounded-full text-sm font-medium border border-blue-100 dark:border-blue-800">
              {{ pos }}
              <button @click="removePosition(index)" type="button"
                class="hover:text-red-500 hover:bg-white dark:hover:bg-gray-800 rounded-full w-4 h-4 flex items-center justify-center transition-colors">
                &times;
              </button>
            </span>
          </div>
      ...
      <div class="mt-6 flex justify-end gap-3 pt-4 border-t dark:border-gray-700">
        <button @click="emit('close')"
          class="px-5 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 font-bold">Hủy</button>
        <button @click="handleSave" class="px-5 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 font-bold">Lưu
          thay đổi</button>
      </div>
    </div>
  </div>
