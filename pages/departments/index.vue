...
const openEditModal = async (dep) => {
  ...
  try {
    ...
    availableEmployees.value = allEmployees.filter(emp => isEmployeeInDepartment(emp, dep.id))
  } catch (error) {
...
const confirmDelete = async (dep) => {
  try {
    const allEmployees = await $fetch('/data/employees.json')
    const employeesInDept = allEmployees.filter(emp => isEmployeeInDepartment(emp, dep.id))
